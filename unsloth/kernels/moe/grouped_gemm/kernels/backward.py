...
@triton.jit
def _grouped_gemm_dX_loop_body(
    expert_idx, m_end, processed_tiles, tidx,
    dY_ptr, w_ptr, dX_ptr, m_sizes_ptr, gather_indices_ptr,
    dY_desc, w_desc, dX_desc,  # Descriptors
    # Constants
    NUM_TOKENS, TOPK, N, K, NUM_SMS, BLOCK_SIZE_M: tl.constexpr, BLOCK_SIZE_N: tl.constexpr, BLOCK_SIZE_K: tl.constexpr,
    # Configs
    PERMUTE_X: tl.constexpr, PERMUTE_Y: tl.constexpr, USE_TMA_LOAD_W: tl.constexpr, USE_TMA_LOAD_dY: tl.constexpr, USE_TMA_STORE: tl.constexpr,
    output_dtype: tl.constexpr, TOTAL_TOKENS
):
...
def _grouped_gemm_dX_kernel(
    ...
    if _TL_RANGE_FLATTEN_SUPPORTED:
        for expert_idx in tl.range(NUM_EXPERTS, flatten = FLATTEN):
            m_end, processed_tiles = _grouped_gemm_dX_loop_body(
                expert_idx, m_end, processed_tiles, tidx,
                dY_ptr, w_ptr, dX_ptr, m_sizes_ptr, gather_indices_ptr,
                dY_desc, w_desc, dX_desc,
                NUM_TOKENS, TOPK, N, K, NUM_SMS, BLOCK_SIZE_M, BLOCK_SIZE_N, BLOCK_SIZE_K,
                PERMUTE_X, PERMUTE_Y, USE_TMA_LOAD_W, USE_TMA_LOAD_dY, USE_TMA_STORE,
